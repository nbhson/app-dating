...
export default function Privacy() {
  const { t } = useI18n();
  const p = (t as any).privacy as any;
  const s = p.sections;
  return (
    <div className="h-[100dvh] overflow-y-auto overscroll-contain bg-[#FFFCFA] no-scrollbar">
      <div className="max-w-3xl mx-auto px-4 md:px-6 py-8 md:py-10 pb-12">
        ...
        <div className="mt-6 glass-strong rounded-[28px] border border-white/60 shadow-[0_12px_40px_rgba(46,26,34,0.08)] p-6 md:p-8 space-y-6">
          ...
          {( [
            [s.collectionTitle, s.collectionBody],
            [s.purposeTitle, s.purposeBody],
            [s.legalTitle, s.legalBody],
            [s.sharingTitle, s.sharingBody],
            [s.locationTitle, s.locationBody],
            [s.retentionTitle, s.retentionBody],
            [s.rightsTitle, s.rightsBody],
            [s.childrenTitle, s.childrenBody],
            [s.changesTitle, s.changesBody],
            [s.contactTitle, s.contactBody],
          ...
          <div className="rounded-2xl bg-[#FFF0F3] border border-[#FCE8EC] p-4 flex items-center gap-3">
            ...
            <div className="text-sm">
              <div className="font-semibold text-[#2E1A22]">support@lumen.app</div>
              <div className="text-xs text-[#8E6B75]">Lumen · Letters, not swipes · {p.lastUpdated}</div>
            </div>
            <Link href="/profile" className="ml-auto px-4 py-2 rounded-full bg-[#2E1A22] text-white text-xs font-semibold">Hồ sơ →</Link>
          </div>
        </div>
        <div className="text-center text-xs font-mono text-[#B08A95] mt-6">Lumen · Privacy · 18+ · Letters, not swipes ♥</div>
      </div>
    </div>
  );
}
